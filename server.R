...
## Set map center at most central city school and zoom
        mapT$setView(c(AucklandSchools$Latitude[507],AucklandSchools$Longitude[507]), zoom = 9)
...
## Create name pop-ups for each of selected schools
        for (i in 1:nrow(Selected))
            mapT$marker(c(Selected$Latitude[i], Selected$Longitude[i]), bindPopup = Selected$Name[i])
        return(mapT)
    })
})
